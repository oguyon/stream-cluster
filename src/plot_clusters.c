...
void print_help(const char *progname) {
    ...
    printf("    2. Anchor points (cluster centers) marked with circles representing the radius limit (rlim).\n");
    ...
    printf("Arguments:\n");
    printf("  <points_file>     The original input text file containing point coordinates (X Y).\n");
    printf("  <log_file>        The log file generated by gric-cluster. It must contain STATS_ and\n");
    printf("                    PARAM_ entries to correctly locate membership files and scale plots.\n");
    printf("  [output_file]     Optional: Path to save the resulting image. If omitted, it defaults\n");
    printf("                    to <points_file> with .png or .svg extension.\n\n");
    printf("Options:\n");
    printf("  -h, --help        Show this help message.\n");
    printf("  -svg              Output SVG image instead of PNG (default: PNG).\n");
    printf("  -fs <size>        Set font size for text labels (default: 18.0).\n\n");
    printf("Example:\n");
    printf("  %s input_points.txt gric_run_log.txt summary_plot.png\n", progname);
}
...
int main(int argc, char *argv[]) {
    ...
    FILE *flog = fopen(log_filename, "r"); if (!flog) { fprintf(stderr, "Error: Could not open log file %s\n", log_filename); return 1; }
    char line[4096];
    ...
    if (output_dir[0]) snprintf(memb_filename, sizeof(memb_filename), "%s/frame_membership.txt", output_dir);
    else strcpy(memb_filename, "frame_membership.txt");

    printf("Reading points: %s\n", points_filename);
    printf("Reading membership: %s\n", memb_filename);

    FILE *f_pts = fopen(points_filename, "r"), *f_memb = fopen(memb_filename, "r");
    if (!f_pts) { fprintf(stderr, "Error: Could not open points file %s\n", points_filename); return 1; }
    if (!f_memb) { fprintf(stderr, "Error: Could not open membership file %s\n", memb_filename); fclose(f_pts); return 1; }
    ...
    while (fgets(lp, 4095, f_pts) && fgets(lm, 1023, f_memb)) {
        ...
        frame_count++;
        if (frame_count % 10000 == 0) { printf("\rProcessing frames: %ld / %ld", frame_count, total_frames); fflush(stdout); }
    }
